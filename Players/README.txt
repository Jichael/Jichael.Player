Dans cette nouvelle itération, un "Player" est une manière bien défini de controller les actions de l'utilisateur.

Il est composé de :

	- Un script PlayerController avec les réglages suivants :
	    - IsDefaultPlayerInScene : défini si ce player est celui qui est utilisé par défaut dans la scène
		...
		- Gravity : la valeur en m/s de la gravité à appliquer
		...
		- MovementSpeedMultiplier : valeur de multiplication pour la vitesse de déplacement
...
Si on veut que le player ignore les collisions, le mettre sur le layer IgnorePhysics créé pour.
