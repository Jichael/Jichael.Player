...
Pour chaque type d'input, une interface qui défini toutes les actions possibles de l'intéraction
	...
	IMouseInteract : Interface pour Clavier/Souris
		...
		- void HoverExit : Comportement à la sortie du curseur de l'objet
			...
			Intéractions de IMouseInteract :
			
				- ChangeCursorOnMouseHover : permet le changement de curseur au survol d'un objet
				- EventsOnClick : liste d'evenements à déclencher lors du clic sur l'objet
				- OutlineOnMouseHover : affiche l'outline lors du survol de l'objet
				- RotationMouse : gère la rotation d'un objet selon des valeurs définies lors du clic sur l'objet
				- SwitchPlayerControllerMouse : change de player lors du clic sur l'objet